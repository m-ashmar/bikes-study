...
def load_cleaned_datasets():
    ...
    data_path = Path("final_project/cleaned_data")
...
def calculate_nearest_bus_stops(start_gdf, end_gdf, metro_bus_stops, shuttle_bus_stops):
    """
    Calculate distance to nearest bus stops
    """
    print("🚌 Calculating nearest bus stops...")
    ...
    # Calculate distances (simplified approach for performance)
    def get_nearest_distance(points_gdf, stops_gdf, max_distance=0.01):  # ~1km
        distances = []
        for idx, point in points_gdf.iterrows():
            point_geom = point.geometry
            min_dist = float('inf')
            
            # Sample nearest stops for performance
            for _, stop in stops_gdf.sample(min(100, len(stops_gdf))).iterrows():
    ...
    start_metro_dist = get_nearest_distance(start_gdf, metro_gdf)
    start_shuttle_dist = get_nearest_distance(start_gdf, shuttle_gdf)
    
    # Calculate distances for end points
    end_metro_dist = get_nearest_distance(end_gdf, metro_gdf)
    end_shuttle_dist = get_nearest_distance(end_gdf, shuttle_gdf)
...
def merge_all_spatial_data():
    """
    Main function to merge all datasets
    """
    print("🚀 Starting spatial data merging process...")
    print("=" * 60)
    ...
    print(f"\n📊 Original trip count: {len(trips_df):,}")
    ...
    # Step 4: Calculate nearest bus stops
    start_bus, end_bus = calculate_nearest_bus_stops(start_gdf, end_gdf, metro_bus_stops, shuttle_bus_stops)
    ...
    output_path = output_dir / "merged_trips.parquet"
...
def validate_merged_data(merged_df, original_count):
    """
    ...
    print(f"  Merged trips: {len(merged_df):,}")
...
if __name__ == "__main__":
    ...
    original_trips = pd.read_parquet("final_project/cleaned_data/daily_bikes_trips.parquet")
    original_count = len(original_trips)
    
    # Perform merging
    merged_df = merge_all_spatial_data()
    
    # Validate results
    validate_merged_data(merged_df, original_count)
    
    print("\n🎯 Spatial merging completed successfully!") 